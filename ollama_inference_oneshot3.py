...
DEFAULT_MODEL_NAME = "qwen3-oneshot-rma3:latest"
DEFAULT_PROMPT_TOKENIZER_NAME = None
LLAMA_MODEL_NAME = "meta-llama/Llama-3.2-3B-Instruct"
ONESHOT_PROMPT_OPTION = "reference-history"
GLM_STOP_SEQUENCES = [
    "<|observation|>",
    "<|system|>",
    "<|user|>",
...
ONESHOT_SYSTEM_PROMPT = (
    ...
    "references. If there are no ambiguous pronouns or omitted references, "
    "rewrited_query may be identical to the query. Then, based on the "
...
"""
python3 /home/hj153lee/RMA/ollama_inference_oneshot3.py \
--model_name phi4-oneshot-rma3:latest \
...
def parse_conversation_history_turns(conversation_history):
    history_items = parse_conversation_history_items(conversation_history)
    turns = []
...
def parse_conversation_history_items(conversation_history):
    if conversation_history is None:
        return []

    if isinstance(conversation_history, list):
        return [str(item) for item in conversation_history]
    else:
        history_text = str(conversation_history).strip()
        try:
            parsed_history = ast.literal_eval(history_text)
            if isinstance(parsed_history, list):
                return [str(item) for item in parsed_history]
            return [str(parsed_history)]
        except (ValueError, SyntaxError):
            return [history_text] if history_text else []


def strip_turn_label(history_item: str) -> str:
    return re.sub(r"^\s*turn\s+\d+\s*:\s*", "", str(history_item), flags=re.IGNORECASE)


def build_reference_conversation_history(conversation_history, reference_turn):
    history_items = parse_conversation_history_items(conversation_history)
    if not reference_turn:
        if len(history_items) == 1:
            return [strip_turn_label(history_items[0])]
        return conversation_history

    reference_match = re.search(r"\bturn\s+(\d+)\b", str(reference_turn), flags=re.IGNORECASE)
    if not reference_match:
        if len(history_items) == 1:
            return [strip_turn_label(history_items[0])]
        return conversation_history

    target_turn = int(reference_match.group(1))
    for item in history_items:
        if re.search(rf"\bturn\s+{target_turn}\s*:", str(item), flags=re.IGNORECASE):
            return [strip_turn_label(item)]
    return conversation_history
...
def infer_oneshot_ollama_model_name(prompt_model_name: str) -> str:
    return f"{infer_model_slug(prompt_model_name)}-oneshot-rma3:latest"
...
def build_oneshot_prompt(
    ...
    reference_conversation_history = build_reference_conversation_history(
    ...
    if is_llama_prompt_model(prompt_model_name):
        prompt_prefix, _ = build_llama_prompts(
            api_str=api_str,
            conversation_history=reference_conversation_history,
            query=example["query"],
            target_json="",
        )
