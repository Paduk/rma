...
usage() {
  cat <<'EOF'
Usage:
  ./run_oneshot2_baseline_train_eval_repeats.sh [options]

Examples:
  ./run_oneshot2_baseline_train_eval_repeats.sh --gpu 3 --profile glm-edge-4b --tags t2

  ./run_oneshot2_baseline_train_eval_repeats.sh --gpu 3 --profile glm-edge-4b --runs 3

  ./run_oneshot2_baseline_train_eval_repeats.sh \
    --gpu 3 \
    --profile qwen2.5 \
    --tags t1,t2,t3 \
    --ollama_host 127.0.0.1:11436
...
Options:
  --profile NAME              train_oneshot_rma2_baseline.py / ollama_inference_oneshot_baseline.py profile. Default: glm-edge-4b
  ...
  --output_prefix PREFIX      Eval filename prefix. Default: <group>-oneshot2-baseline
...
if [[ -z "$OUTPUT_PREFIX" ]]; then
  OUTPUT_PREFIX="${RESULT_GROUP}-oneshot2-baseline"
fi
...
for tag in "${tags[@]}"; do
  ...
  output_tag="$tag"
  ollama_model_name="${MODEL_SLUG}-oneshot2-baseline-${tag}"
  artifact_stem="${MODEL_SLUG}-rma-oneshot2-baseline-${tag}"
  ...
  train_cmd=(python3 train/train_oneshot_rma2_baseline.py
  ...
  eval_cmd=(python3 ollama_inference_oneshot_baseline.py
  ...
  echo
  echo "=== One-shot baseline run tag: $tag ==="
