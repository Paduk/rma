...
def resolve_directory_summary_output_path(input_dir):
    input_dir = Path(input_dir)
    return input_dir / f"{input_dir.name}.macro_summary.tsv"


def list_input_tsv_files(input_dir):
    input_dir = Path(input_dir)
    return sorted(
        path
        for path in input_dir.glob("*.tsv")
        if path.is_file() and not path.name.endswith(".macro_summary.tsv")
    )
...
def export_directory_human_readable_summary(
    input_dir, metric_col="all", turn_weight_scheme=None, reverse=False
):
    input_files = list_input_tsv_files(input_dir)
    if not input_files:
    ...
    for input_file in input_files:
        ...
        if "turn" not in df.columns:
            print(f"Skipping {input_file.name}: missing `turn` column.")
            continue
    ...
    if not summary_frames:
        raise ValueError(
            f"No valid TSV files with a `turn` column found in directory: {input_dir}"
        )

    combined_df = pd.concat(summary_frames, ignore_index=True, sort=False)
    combined_df = format_summary_df_for_export(combined_df)
    output_path = resolve_directory_summary_output_path(input_dir)
    combined_df.to_csv(output_path, sep="\t", index=False)
    return output_path, processed_files
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        "-i",
        required=True,
        help="Path to a result TSV file or a directory containing result TSV files.",
    )
    parser.add_argument(
        "--turn-weight-scheme",
...
def main():
    ...
    if input_path.is_dir():
        summary_output_path, processed_files = export_directory_human_readable_summary(
            input_path,
            metric_col="all",
            turn_weight_scheme=args.turn_weight_scheme,
            reverse=args.reverse,
        )
        print(f"Processed {len(processed_files)} TSV files from: {input_path}")
        print(f"Combined Summary TSV exported to: {summary_output_path}")
        return
