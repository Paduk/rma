...
DEFAULT_DATA_PATHS = [
    Path(__file__).resolve().parent / "datasets/manual/ad/rewrite-qwen25.tsv",
    Path(__file__).resolve().parent / "datasets/manual/ad/rewrite-qwen25.tsv",
    #Path(__file__).resolve().parent / "datasets/result/base_rewrite_gemini.tsv",
]
    ...
    for path in paths:
        with open(path, newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if fname == "it2_NR_tc.tsv":
                    continue
                turn = None
                for prefix, t in [("it2", "2"), ("it3", "3"), ("it4", "4"), ("it5", "5")]:
                    if fname.startswith(prefix):
                        turn = t
                        break
                if turn is None:  # turn을 알 수 없으면 스킵
                    continue
...
if __name__ == "__main__":
    # 예: python statistic.py RMA/datasets/result/qwen3-rma-base.tsv RMA/datasets/result/qwen3-rma-complex.tsv
    if len(sys.argv) > 1:
        paths = sys.argv[1:]
    else:
