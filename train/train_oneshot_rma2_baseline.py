...
DEFAULT_OUTPUT_TAG = "oneshot-rma2-baseline"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a one-shot RMA v2 baseline LoRA adapter without tool/candidate prompt input."
    )
    parser.add_argument(
        "--profile",
...
def apply_postprocess_defaults(args, model_name: str, output_root: Path):
    ...
    artifact_stem = f"{model_slug}-rma-oneshot2-baseline"
    ...
    if not args.ollama_model_name:
        args.ollama_model_name = f"{model_slug}-oneshot2-baseline"
