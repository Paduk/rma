...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Task-specific subdirectory under output_root/model_name.",
    )
    parser.add_argument(
        "--max_length",
        type=int,
...
def is_smollm3_tokenizer(tokenizer) -> bool:
    tokenizer_names = [
...
def apply_postprocess_defaults(args, output_root: Path, model_name: str):
    ...
    artifact_stem = f"{model_slug}-{profile_slug}-{args.output_tag}-{prefix}"
...
model_root_dir = output_root / model_dirname
output_dir = model_root_dir / args.output_tag
...
print(f"output_tag: {args.output_tag}")
