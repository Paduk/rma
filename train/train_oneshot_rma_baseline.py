...
from oneshot_qwen_prompt import render_chat_template, render_messages_as_plain_text
...
DEFAULT_OUTPUT_TAG = "oneshot-rma-baseline"
ONESHOT_SYSTEM_PROMPT = (
    "Given a conversation history and a user query, first rewrite the query by "
    "resolving ambiguous references using the conversation history. Then predict "
    "the next action plan and generate its arguments. Return compact JSON only with keys "
    "\"rewrited_query\", \"plan\", and \"arguments\". Always include all three "
    "keys. The value of \"arguments\" must always be an object. If no action is "
    "appropriate, set \"plan\" to \"None\" and \"arguments\" to {}."
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a one-shot RMA baseline LoRA adapter without tool/candidate prompt input."
    )
    parser.add_argument(
        "--profile",
...
def build_llama_prompts(example, target_json: str):
    user_content = (
        build_oneshot_user_content(example)
    )
    prompt_prefix = (
...
def build_epoch_eval_callbacks(args, tokenizer, model_name: str, max_length: int):
...
def apply_postprocess_defaults(args, model_name: str, output_root: Path):
    ...
    artifact_stem = f"{model_slug}-rma-oneshot-baseline"
    ...
    if not args.ollama_model_name:
        args.ollama_model_name = f"{model_slug}-oneshot-baseline"
