...
usage() {
  cat <<'EOF'
Usage:
  ./run_oneshot_train_eval_repeats.sh [options]

Examples:
  ./run_oneshot_train_eval_repeats.sh --gpu 3 --profile glm-edge-4b --tags t2

  ./run_oneshot_train_eval_repeats.sh --gpu 3 --profile glm-edge-4b --runs 3

  ./run_oneshot_train_eval_repeats.sh \
    --gpu 3 \
    --profile granite3.3-2b \
    --tags t1,t2,t3 \
    --ollama_host 127.0.0.1:11437

  ./run_oneshot_train_eval_repeats.sh --gpu 3 --profile qwen2.5 --tags t1,t2,t3
...
Options:
  --profile NAME              train_oneshot_rma2.py / ollama_inference_oneshot.py profile. Default: glm-edge-4b
...
if [[ -z "$OUTPUT_PREFIX" ]]; then
  OUTPUT_PREFIX="${RESULT_GROUP}-oneshot"
fi
...
for tag in "${tags[@]}"; do
  ...
  output_tag="$tag"
  ollama_model_name="${MODEL_SLUG}-oneshot-${tag}"
  artifact_stem="${MODEL_SLUG}-rma-oneshot-${tag}"
  ...
  train_cmd=(python3 train/train_oneshot_rma2.py
  ...
  eval_cmd=(python3 ollama_inference_oneshot.py
