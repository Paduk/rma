...
def build_reference_turn(source_file, conversation_history):
    ...
    file_name = os.path.basename(str(source_file))
...
def get_data_files():
    return {   
        "train-test": [
            "datasets/train/it2_nonNR_train.tsv",   
...
def is_smollm3_tokenizer(tokenizer):
    tokenizer_names = [
        getattr(tokenizer, "name_or_path", ""),
        getattr(tokenizer, "_name_or_path", ""),
        getattr(tokenizer, "init_kwargs", {}).get("name_or_path", "")
        if isinstance(getattr(tokenizer, "init_kwargs", {}), dict)
        else "",
    ]
    return any("smollm3" in str(name).lower() for name in tokenizer_names)


def with_smollm3_system_override(tokenizer, messages):
    if not is_smollm3_tokenizer(tokenizer):
        return messages
...
def render_chat_template(tokenizer, messages, add_generation_prompt):
    messages = with_smollm3_system_override(tokenizer, messages)
    kwargs = {
        "tokenize": False,
...
def build_rewrite_prompt(example, model_config, rewrite_tokenizer, chat_template_fallback, source_file=None):
    data = {
...
def main():
    ...
    printed_first_rewrite_prompt = False
    printed_first_plan_prompt = False
    ...
    for test_key in test_keys:
        ...
        for file_path in data_files[test_key]:
            ...
            for ex in tqdm(ds, desc=f"Processing {test_key}/{os.path.basename(file_path)}"):
                ...
                if not printed_first_rewrite_prompt:
                    print("\n# First rewrite prompt\n")
                    print(rewrite_prompt)
                    print()
                    printed_first_rewrite_prompt = True
                ...
                if not rewrite_error:
                    try:
                        ...
                        if not printed_first_plan_prompt:
                            print("\n# First planning prompt\n")
                            print(plan_prompt)
                            print()
                            printed_first_plan_prompt = True
